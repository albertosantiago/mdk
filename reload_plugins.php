...
mdk_plugins_set_tabs("RELOAD_REINSTALL");
?>
<div>
<?php 
if((!empty($_POST['action']))&&($_POST['action']=="reinstall")){
?>
<input type="button" onclick="window.location='reload_plugins.php'" style="float:right" value="Volver"/>
<br/><br/>
		...
		foreach($pluginTree as $pluginType => $plugins){
			...
			foreach($plugins as $plugin){
				if($plugin->source != "std"){
					...
					$html .= "<li style='border-bottom:1px solid #ccc;width:320px;padding:10px;'><span style='width:300px;display:block;float:left;clear:left;'>$plugin->displayname</span><input type='checkbox' value='$pluginName' name=\"plugins[]\" $checked /></li>";
				}		
			}
			if(!empty($html)){
